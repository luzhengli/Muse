...
面向自媒体创作者的一站式创作工厂（本地优先、单用户，v0.2「沉浸式写作与本地配置」）。覆盖完整创作闭环：
...
| 选题板 | `/topics` | 基于素材集合生成选题卡片、创作 brief、一键生成初稿 |
| 写作台 | `/articles/[id]` | 沉浸式 Markdown 编辑（代码块语言高亮、KaTeX 公式、表格、任务列表）、`/` 插入菜单、选区 Bubble Menu 与 AI 扩写/改写/重组（选区映射保护）、自动保存工作稿 + 显式版本检查点、专注模式、Markdown 导入/导出 |
