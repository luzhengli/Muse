...
面向自媒体创作者的一站式创作工厂（本地优先、单用户，v0.4「可信创作飞轮」建设中：feat-020/021/022 已完成，feat-023 起待实现）。覆盖完整创作闭环：
...
| 写作台 | `/articles/[id]` | 沉浸式 Markdown 编辑、自动保存工作稿 + 显式/下游自动检查点、版本与下游过期状态；资料面板搜索语料块并插入证据引用（点击正文引用文字查看「这句话有什么依据」，来源变化/删除自动降级提示），可编辑 Brief 并先预览后确认生成初稿 |
