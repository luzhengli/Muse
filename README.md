...
面向自媒体创作者的一站式创作工厂（本地优先、单用户，v0.4「可信创作飞轮」建设中：feat-020~023 已完成，feat-024~026 待实现）。覆盖完整创作闭环：
...
| 写作台 | `/articles/[id]` | 沉浸式 Markdown 编辑、自动保存工作稿 + 显式/下游自动检查点、版本与下游过期状态；顶部 NextAction 条按事实计算成稿就绪（自然语言状态、唯一下一步、阻塞原因、可跳过风险、待办直达）；资料面板搜索语料块并插入证据引用（点击正文引用文字查看「这句话有什么依据」，来源变化/删除自动降级提示），可编辑 Brief 并先预览后确认生成初稿 |
| 审阅台 | `/articles/[id]/review` | AI 六维审阅（事实/结构/风格/安全/合规/润色）+ AI 事实检查（区分资料支持/缺少资料/资料冲突/来源不可用）+ 人工意见，可接受/忽略 |
| 包装台 | `/articles/[id]/packaging` | 标题候选、摘要、封面/配图提示词、图文卡片、本地图片管理 |
| 平台适配 | `/articles/[id]/variants` | 派生小红书 / X / 微信公众号版本（标题、标签、CTA、发布说明） |
| 发布中心 | `/publish` | 定时任务、状态跟踪、失败重试；创建与执行均经服务端就绪校验，基于旧正文的平台稿不可发布；mock 发布器 + 可扩展平台适配器接口 |
