...
import { db, materials, collections, collectionMaterials } from "@/db";
        ...
        {rows.length === 0 && (
          <Card className="col-span-2">
            <CardContent className="py-10 text-center text-sm text-(--color-muted)">
              {q || tag ? "没有匹配的素材。" : "素材库还是空的，从上方导入第一条素材开始。"}
            </CardContent>
          </Card>
        )}
        {rows.map((m) => (
            ...
            <Card className="h-full transition-colors hover:border-(--color-primary)">
              <CardContent className="space-y-2 p-4">
                ...
                <p className="line-clamp-2 text-xs text-(--color-muted)">
                  {snippetByMaterial.get(m.id) ?? m.summary ?? ""}
                  {!snippetByMaterial.get(m.id) && !m.summary && m.rawContent.slice(0, 100)}
                </p>
